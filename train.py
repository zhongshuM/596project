...
def train(model, criterion, dataloader, dataloader_val, optimizer, scheduler, cfg, device):
    ...
    for epoch in range(cfg.epochs):
        ...
        if epoch % 3 == 0:
            if not os.path.isdir(f'./checkpoints/train_{cfg.model_name}'):
                 os.makedirs(f'./checkpoints/train_{cfg.model_name}')
            torch.save(model.state_dict(),
                    f'./checkpoints/train_{cfg.model_name}/{epoch}.pt')
        # run one validation
        with torch.no_grad():
...
def parse():
    ...
    parser.add_argument('--model_name', type=str, default='sunrgbd', required=True, help='the model name')
    parser.add_argument('--data', type=str, default='SUNRGBD', choices=['Structured3D', 'SUNRGBD'])
    parser.add_argument('--pretrained', type=str, default='./Structured3D_pretrained.pt', help='the pretrained model')
...
if __name__ == '__main__':
    ...
    dataloader_val = torch.utils.data.DataLoader(
        dataset_val, batch_size=4, shuffle=False, num_workers=1)
