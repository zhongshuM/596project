...
def ConvertLayout(img, ups, downs, attribution, pwalls=None, pfloor=None, pceiling=None, K=None, ixy1map=None, oxy1map=None, valid=None, pixelwise=None):
    ...
    ax = Axes3D(fig)
    tmp = np.array(_3ds[0])
    tmp1 = np.array(_3ds[1])
    tmp2 = np.array(_3ds[2])
    ax.scatter(tmp[:, 0], tmp[:, 1], tmp[:, 2], c='r')
    ax.scatter(tmp1[:, 0], tmp1[:, 1], tmp1[:, 2], c='g')
    ax.scatter(tmp2[:, 0], tmp2[:, 1], tmp2[:, 2], c='b')
    plt.show() """
    # plt.plot(_3ds[0], projection="3d")
    #pclpy.pcl.io.savePCDFileASCII(f'results/{iters}_polys.pcd', *polys)
...
def DisplayMeshLayout(ups, downs, attribution, pwalls, pceiling, pfloor, ixy1map, K_inv):
    ...
    if attribution == 1:  # only ceiling
        ...
        maxdy = max(np.max(downs[:, 1]) + 10, h+1)
        print("ups: ", ups.shape)
        print("downs: ", downs.shape)
        print("pceiling: ", pceiling)
        print("pfloor: ", pfloor)
        # draw floor and ceiling
        if len(pceiling) > 0:
            ...
            print("contour ceil: ", contour_.shape)
            _2d, _3d = Convert2DTo3D(contour_, ixy1map, pceiling[0], K_inv)
            _2ds.append(_2d)
            _3ds.append(_3d)
        if len(pfloor) > 0:
